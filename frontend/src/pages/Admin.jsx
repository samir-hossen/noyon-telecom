...
const emptyCoupon = { code: '', type: 'percent', value: '', minSubtotal: '' };
...
export default function Admin() {
  ...
  function loadAnalytics() {
    api.get('/admin/analytics').then((d) => setAnalytics(d));
  }
  function loadCoupons() {
    api.get('/admin/coupons').then((d) => setCoupons(d.coupons));
  }
  function loadMe() {
    api.get('/auth/me').then((d) => setTwoFA({ enabled: !!d.user.twoFAEnabled }));
  }
  function loadAuditLogs() {
    api.get('/admin/audit-logs').then((d) => setAuditLogs(d.logs));
  }
  ...
  function loadQuotes(status = quoteFilter) {
    api.get(`/admin/quotes${status ? `?status=${status}` : ''}`).then((d) => setQuotes(d.quotes));
  }
  function loadLowStock() {
    api.get('/admin/inventory/low-stock').then((d) => setLowStock(d.products));
  }
                  ...
                  {products.map((p) => (
                    <tr key={p.id}>
                      <td data-label="Product" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <img src={resolveImg(p.img)} alt={p.name} loading="lazy" decoding="async" style={{ width: 36, height: 36, objectFit: 'cover' }} />
                        {p.name}
                        {p.published === false && (
          ...
          <form onSubmit={onCreateCoupon} className="form-panel wide" style={{ marginBottom: 30 }}>
            ...
            <div className="field-row">
              <div className="field">
                ...
                <input required type="number" step="0.01" value={couponForm.value} onChange={(e) => setCouponForm({ ...couponForm, value: e.target.value })} />
              </div>
              <div className="field">
