...
// Every customer-facing product read (storefront listing/detail, cart,
// wishlist) needs to go through this — never send a bare serializeProduct()
// result straight to a customer response. serializeProduct() always
// includes the raw `dealerPrice` field (the admin panel needs it to manage
// wholesale pricing), and until now nothing stripped it back out again
// before a customer-facing response went out. The frontend never displays
// that field, so this was invisible in normal use, but it was sitting in
// every product API response's raw JSON regardless — anyone opening
// DevTools' Network tab (a curious customer, or a competitor) could read
// every product's wholesale price straight off the wire. Approved dealers
// keep seeing their own dealerPrice back (it's legitimately theirs to see);
// everyone else — guests, regular customers, even a pending/rejected
// dealer application — gets it stripped entirely.
export function withViewerPricing(product, user, qty = 1) {
  const price = priceForViewer(product, user, qty); // needs the *original* product.dealerPrice internally, so this runs before stripping it below
  const isApprovedDealer = user?.role === 'dealer' && user?.dealerStatus === 'approved';
  const { dealerPrice, ...publicFields } = product;
  return {
    ...publicFields,
    ...(isApprovedDealer ? { dealerPrice } : {}),
