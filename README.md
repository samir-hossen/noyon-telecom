...
- In-process cache-aside on the homepage/catalog read endpoints (`backend/src/utils/cache.js`) — no Redis/external service required; swap-in point documented there if you later scale to more than one backend instance
- Optional typo-tolerant search via Meilisearch (`backend/src/utils/search.js`) — a drop-in enhancement over the default Postgres search, on only once `MEILISEARCH_HOST` is set; see the Maintenance Recommendations section of `DEPLOYMENT.md`
